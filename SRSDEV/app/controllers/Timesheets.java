...
public class Timesheets extends Controller {
	...
	public static Result listForHeadOffice(int page, String sortBy,
			String order, String filter, String pageAction) {
		Logger.info("@C Timesheets -->> listForHeadOffice(" + page + ","
		...
		// check for role
		String email = session("email");
		Logger.debug("@C Timesheets -->> listForHeadOffice(" + page + ","
				+ sortBy + "," + order + "," + filter + "," + pageAction
				+ ") -->>Employee email: " + email);

		String role = session("role");
		Logger.debug("@C Timesheets -->> listForHeadOffice(" + page + ","
				+ sortBy + "," + order + "," + filter + ") -->>Role : " + role);
		...
		if (filter == "") {
			emp = Employee.findByEmail(email);
			Logger.info("@C listForHeadOffice====> employee username ----------->: " + email
					+ " &&&&&&&emp id :" + emp.id);
			Long empId = emp.id;
			return ok(listForHeadOffice.render(Timesheet.page(page, 10,
					colName, value, sortBy, order, filter, empId), sortBy,
					order, filter, store, pageAction));
		} else {
	...
	public static Result editFormForHeadOffice(Long tid, Long sid, String action) {
		...
		Logger.info("@C Timesheets -->> editFormForHeadOffice(" + tid + ","
				+ sid + ") -->>");
		Store store = Store.find.byId(sid);
		Timesheet timesheet = Timesheet.find.byId(tid);
		Form<Timesheet> timeSheetForm = form(Timesheet.class).fill(timesheet);
		...
		Logger.info("@C Timesheets -->> editFormForHeadOffice(" + tid + ","
				+ sid + ") <<--");
		return ok(editFormForHeadOffice.render(timeSheetForm, timesheet, store,
				emp));
	}
	 ...
	 * flash("pageAction", action);
	 * 
	 * if (action.equals("ADD")) { Employee emp =
	 * Employee.findByEmail(session("email"));
	 * 
	...
	public static Result showOptionsForHeadOffice(String page, String action) {
		...
		if (action.equals("ADD")) {
			...
			return redirect(routes.Timesheets.listForHeadOffice(0, "status",
					"asc", "", action));
	...
	// method used to select Page in Admin module to Process Head Office
	// Timesheets
	...
	// used to get List of Head Office Timesheet's Based on the select Criteria
	// in showSelectPageForHeadOfficeTimesheet page.

	public static Result getHeadOfficeTimesheets() {

		Long empId = Long.parseLong(form().bindFromRequest().get("empId"));
		// String weekDate = form().bindFromRequest().get("timesheetWeekDate");
		String tmsStartDate = form().bindFromRequest()
		...
		int days = Days.daysBetween(new DateTime(startDate),
				new DateTime(endDateNext)).getDays(); // => 34

		Logger.info("@C Timesheets -->> getHeadOfficeTimesheets(" + startDate
				+ "+" + endDateNext + "+" + days + ") <<--");

		return ok(timesheetListOfHeadofficeForAdmin.render(startDate, endDate,
				emp, empId, days - 1, stDate, 0));
	}
			...
			if (t.SIZE > 0) {
				...
				for (int i = 0; i < t; i++) {
					...
					if (startTimeHour != null) {
						Timesheet tm = Timesheet.savetimeSheet(empId,
								startDate, endDate, leaveType, startTimeHour,
								startTimeMins, endTimeHours, endTimeMins,
								duration, activity, jobTitle, firmType,
								storeId, status);
	...
	public static Result showSelectPageForVerify(String date) {
		List<Employee> empList = Employee
				.getHeadOfficeEmployeeForNodateStr(1L);
	Logger.info("date is :"+date);
		List<Timesheet> totalSubTimesheetList = Timesheet
				.getHeadOfficeEmployeeTimeSheetForSubmitted(1L);
		Logger.info("time sheet list : =============> "+totalSubTimesheetList.size());
		// String date = "no_date";

		return ok(showSelectPageForHOTVerification.render(empList,
				totalSubTimesheetList, date));
	}

	public static Result getAllHeadOfficeTimesheetsToVerify() {

		String weekDate = form().bindFromRequest().get("timesheetStartDate");
		Date startDate = Application.getDate(weekDate);
		Date endDate = Application.getNextDate(weekDate, 6);
		System.out.println("end Date=============: " + endDate);
		List<Employee> empList = Employee
				.getHeadOfficeEmployeeForSubmittedTimeSheets(1L,startDate,endDate);
		List<Timesheet> totalSubTimesheetList = Timesheet
				.getHeadOfficeEmployeeTimeSheetForSubmittedByWeekDate(1L,startDate,endDate);
		System.out.println("startt Date=============: " + startDate);
		return ok(showSelectPageForHOTVerification.render(empList,
				totalSubTimesheetList, weekDate));
	}

	public static Result getHeadOfficeTimesheetsForVerify() {
		 Long empId = Long.parseLong(form().bindFromRequest().get("empIdForGetHeadOfficeTimesheetsForVerifyMethod"));
		 String weekDate = form().bindFromRequest().get("stDateForGetHeadOfficeTimesheetsForVerifyMethod");
		Date startDate = Application.getDate(weekDate);
		Date endDate = Application.getNextDate(weekDate, 6);
		Date sDate = Application.getNextDate(weekDate, 6);
		System.out.println("end Date=============: " + weekDate);
		System.out.println("startt Date=============: " + sDate);
		System.out.println("end Date=============: " + endDate);
		System.out.println("startt Date=============: " + startDate);
		return ok(listPageForHOTVerification.render(startDate, endDate, empId));
	}

	public static Result updateWeeklyHOTimesheetsByWeekDate(String weekStartDate) {

		int empListSize = Integer.parseInt(form().bindFromRequest().get(
				"empListSize"));
		for (int i = 0; i < empListSize; i++) {
			String emp = "empId_" + i;
			Long empId = Long.parseLong(form().bindFromRequest().get(emp));
			System.out.println("emp Id :" + empId);
			Timesheet.updateStatusSubToApprovedByWeekDate(empId,
					Application.getDate(weekStartDate),
					Application.getNextDate(weekStartDate, 6));
		}
		List<Employee> empList = Employee
				.getHeadOfficeEmployeeForSubmittedTimeSheets(1L,Application.getDate(weekStartDate),Application.getNextDate(weekStartDate,6));
		List<Timesheet> totalSubTimesheetList = Timesheet
				.getHeadOfficeEmployeeTimeSheetForSubmitted(1L);
		System.out.println("emp list size :" + empListSize);
		return ok(showSelectPageForHOTVerification.render(empList,
				totalSubTimesheetList, weekStartDate));

	}

	public static Result verifyHOTimesheet() {
		String stDate=form().bindFromRequest().get("stDateForVerifyHOTimesheet");
		String enDate=form().bindFromRequest().get("enDateForVerifyHOTimesheet");
		long empId=Long.parseLong(form().bindFromRequest().get("empIdForVerifyHOTimesheet"));
		Employee emp = Employee.find.byId(empId);
		Date sDate = Application.getDate(stDate);
		Date eDate = Application.getDate(enDate);
		String result = Timesheet.updateVerifiedHOList(sDate, eDate, empId);
		List<Employee> empList = Employee
				.getHeadOfficeEmployeeForSubmittedTimeSheets(1L,sDate,eDate);
		List<Timesheet> totalSubTimesheetList = Timesheet
				.getHeadOfficeEmployeeTimeSheetForSubmitted(1L);
		System.out.println("emp list size :" + empList.size());
		flash("success", "Timesheet Approved for Employee " + emp.firstName);

		 return ok(showSelectPageForHOTVerification.render(empList,
					totalSubTimesheetList, stDate));
	}

	public static Result updateRejectedStatusForHOTimesheets() {
		String weekStartDate=form().bindFromRequest().get("stDateForUpdateRejectedStatusForHOTimesheetsMethod");
		Long empId=Long.parseLong(form().bindFromRequest().get("empIdForUpdateRejectedStatusForHOTimesheetsMethod"));
		String result = "";
		Employee emp = Employee.find.byId(empId);
		System.out.println("eddddddddddddv   :" + weekStartDate);

		Date sDate = Application.getDate(weekStartDate);

		Date eDate = Application.getNextDate(sDate, 6);
		result = Timesheet.updateRejectedListInHOTimesheet(sDate, eDate, empId);
		List<Employee> empList = Employee
				.getHeadOfficeEmployeeForSubmittedTimeSheets(1L,sDate,eDate);
		List<Timesheet> totalSubTimesheetList = Timesheet
				.getHeadOfficeEmployeeTimeSheetForSubmitted(1L);
		System.out.println("emp list size :" + empList.size());
		flash("success", "Timesheet Rejected for Employee " + emp.firstName
				+ ",Please recheck");
	...
	public static Result editFormForHeadOfficeRejectedTimesheetList(
			Long timeSheetId, Long empId, Long storeId, String pageAction) {

		flash("action", "timesheet_HO");
		flash("pageAction", pageAction);
		Logger.info("@C Timesheets -->> editFormForHeadOfficeRejectedTimesheetList("
				+ timeSheetId + "," + storeId + ") -->>");
		Store store = Store.find.byId(storeId);
		Timesheet timesheet = Timesheet.find.byId(timeSheetId);
		Form<Timesheet> timeSheetForm = form(Timesheet.class).fill(timesheet);
		Employee emp = Employee.find.byId(timesheet.empid);
		...
		return ok(editformForHOTRejectectedTimesheetList.render(timeSheetForm,
				timesheet, store, emp));

	}

	public static Result updateEditListForRejectedTS(Long tmId, Long storeId) {
		Long empId = Long.parseLong(form().bindFromRequest().get("empid"));
		String startDate = form().bindFromRequest().get("hoEditStartDate");
		String endDate = form().bindFromRequest().get("hoEditEndDate");
		String stHours = form().bindFromRequest().get("hoEditStartTimeHour");
		String stMins = form().bindFromRequest().get("hoEditStartTimeMins");
		String endHours = form().bindFromRequest().get("hoEditEndTimeHour");
		String endMins = form().bindFromRequest().get("hoEditEndTimeMins");
		String duration = form().bindFromRequest().get("durationInHoEdit");
		String activity = form().bindFromRequest().get("hOTEditTextarea");
		String leaveType = form().bindFromRequest().get("leaveTypeInHOEdit");

		/*Logger.info("@C ====>updateEditListForRejectedTS" + "TIMESHEETID : "
				+ tmId);
		Logger.info("@C ====>updateEditListForRejectedTS" + "storeId : "
				+ storeId);
		Logger.info("@C ====>updateEditListForRejectedTS" + "empId" + empId);
		Logger.info("@C ====>updateEditListForRejectedTS" + "START hours : "
				+ stHours);
		Logger.info("@C ====>updateEditListForRejectedTS" + "start min : "
				+ stMins);
		Logger.info("@C ====>updateEditListForRejectedTS" + "end hours : "
				+ endHours);
		Logger.info("@C ====>updateEditListForRejectedTS" + "end mins : "
				+ endMins);
		Logger.info("@C ====>updateEditListForRejectedTS" + "duration : "
				+ duration);
		Logger.info("@C ====>updateEditListForRejectedTS" + "activity : "
				+ activity);
		Logger.info("@C ====>updateEditListForRejectedTS" + "leaveType : "
				+ leaveType);*/
		if(tmId.equals(null)){
			flash("success","No timesheet found");
			return redirect(routes.Timesheets.listForHeadOffice(0, "status",
					"asc", "", "VIEW"));
		}
		else{
			if(leaveType.equals("None")){
				Timesheet
				.updateListForHOTInEditPage(tmId, storeId, empId, startDate,
						endDate, stHours, stMins, endHours, endMins, duration,
				activity,leaveType);
			}
			else{
				Timesheet.updateHoEmployeeLeaveTypeInEditForm(tmId, leaveType);
			}
