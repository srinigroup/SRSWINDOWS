...
public class Application extends Controller {
	...
	public static Result index() {
		...
		else if (role.contains("PEM")) { // render to Management page
			return ok(home_for_petroformEmployees.render());
		}
	...
	public static Result checkHeadOfficeTimesheetExistsInEditPage(Long sId,
			Long empId, String date, String endDate, String leaveType,
			String startHours, String startMins, String endHour,
			String endMins, Long tid) {

		Logger.info("@C Application -->> checkHeadOfficeTimesheetExistsInEditPage() -->> ");
		...
		Logger.debug("@C Application -->> checkHeadOfficeTimesheetExistsInEditPage() -->> Last Wednesday...."
				+ lastWednesday);
		Logger.info("store id inside checkHeadOfficeTimesheetExistsInEditPage"
				+ sId);
		Logger.info("employee id inside checkHeadOfficeTimesheetExistsInEditPage"
				+ empId);
		Logger.info("start date inside checkHeadOfficeTimeSheetExistOrNot"
				+ date);
		Logger.info("endDate inside checkHeadOfficeTimesheetExistsInEditPage"
				+ endDate);
		Logger.info("leaveType inside checkHeadOfficeTimesheetExistsInEditPage"
				+ leaveType);
		Logger.info("startHours inside checkHeadOfficeTimesheetExistsInEditPage"
				+ startHours);
		Logger.info("startMins inside checkHeadOfficeTimesheetExistsInEditPage"
				+ startMins);
		Logger.info("endHours inside checkHeadOfficeTimesheetExistsInEditPage"
				+ endHour);
		Logger.info("endMins inside checkHeadOfficeTimesheetExistsInEditPage"
				+ endMins);
		Logger.info("@C checkHeadOfficeTimesheetExistsInEditPage------------timesheet id >"
				+ tid);
		if (lastWednesday.compareTo(Application.getDate(new Date())) >= 0) {
		...
		if (newStart.compareTo(lastWednesday) <= 0
			...
			// end date
			List<Timesheet> timesheetList = Timesheet
					.checkHeadOfficeTimesheetExistsInEditForm(sId, empId, date,
							endDate, startHours, startMins, endHour, endMins,
							tid);
	...
	public static Result checkHOTTimesheetExistsInEditPageForLeave(
			Long sId, Long empId, String date, String endDate, Long tid,
			String leaveType) {
		Logger.info("@C Application -->> checkHOTTimesheetExistsInEditPageForLeave() -->> ");
		int count = 0;
		...
		Logger.debug("@C Application -->> checkHOTTimesheetExistsInEditPageForLeave() -->> Last Wednesday...."
				+ lastWednesday);

		if (lastWednesday.compareTo(Application.getDate(new Date())) >= 0) {

			lastWednesday = Application.getNextDate(lastWednesday, -7);
			Logger.debug("@C Application -->> checkHOTTimesheetExistsInEditPageForLeave() -->> if wednesday of week is greater than todays date"
					+ lastWednesday);
		}
		...
		if (newStart.compareTo(lastWednesday) <= 0
			...
			List<Timesheet> timesheetList = Timesheet
					.checkTimesheetExistsInHoEmployeeEditForLeave(sId, empId, date, endDate,tid);
	...
	public static Result checkUrl(String filePath){
		String email = session("email");
		String role = session("role");
		if (email == null || role == null) {
		return ok(AuthenticatePage.render());
		}
		
		return redirect(routes.Assets.at("/INVOICEPROCESSINGSYSTEM/"+filePath));
	}
	...
	public static Result javascriptRoutes() {
		...
		return ok(Routes.javascriptRouter("myJsRoutes",
				...
				routes.javascript.Application
						.checkHeadOfficeTimesheetExistsInEditPage(),
				routes.javascript.Application
						.checkHOTTimesheetExistsInEditPageForLeave(),
