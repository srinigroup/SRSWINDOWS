...
public class Timesheet extends Model {
	...
	public static Page<Timesheet> page(int page, int pageSize, String colName,
			Long value, String sortBy, String order, String filter, Long empId) {
		Logger.info("@M Timesheet -->> page() -->>");
		return (find.where().eq("empid", empId)
				.ilike("status", "%" + filter + "%")
				// .gt("timesheet.date",lastWeek)
				.eq(colName, value).orderBy(sortBy + " " + order)
				.findPagingList(pageSize)).setFetchAhead(false).getPage(page);
	...
	public static List<Timesheet> getHOTimesheetbyEmployee(Long empId,
			Date startDate, Date endDate) {
	...
	public static List<Timesheet> getHOTimesheetByEmployeeAndDate(Long sid,
			Long empId, Date startDate) {

		Logger.info("@M Timesheet -->> getTimesheetsStoreDuration(" + sid + ","
				+ empId + "," + startDate + ") -->> Gives list of Timesheets");
		return (find.where().eq("firmid", sid).eq("empid", empId)
				.eq("status", "SUBMITTED").ge("date", startDate)
				.lt("date", Application.getNextDate(startDate, 1))
				.orderBy("date asc").findList());
	...
	public static void updateStatusSubToApprovedByWeekDate(Long empId,
			Date sDate, Date eDate) {
		List<Timesheet> tmList = new ArrayList<Timesheet>();
		tmList = find.where().eq("empid", empId).eq("status", "SUBMITTED")
				.ge("date", sDate).le("end_date", eDate).findList();
		System.out.println("tmlist :" + tmList.size());
		for (int i = 0; i < tmList.size(); i++) {
			Timesheet tm = Timesheet.find.byId(tmList.get(i).id);

			System.out.println("tm id ======= :" + tm.id);
	...
	public static List<Timesheet> getHeadOfficeTimesheetByEmployeeAndDateForVerify(
			Long empId, Date startDate, int i) {
		...
		timeSheetList = find.where().eq("empid", empId)
				...
				.ge("date", Application.getDate(startDate))
				.lt("date", Application.getNextDate(startDate, i))
				.orderBy("date asc").findList();
	...
	public static String getTotalHOTimesheetHours(Long empId, Date startDate) {
		Logger.info("@M Timesheet -->> getTotalTimesheetHours(" + empId + ","
				+ startDate.getDate() + ") -->>");

		Date endDate = Application.getNextDate(startDate, 6);

		List<Timesheet> timesheetList = getHOTimesheetbyEmployee(empId,
				startDate, endDate);
	...
	public static List<Timesheet> checkHeadOfficeTimesheetExists(Long sid,
		...
		return (find
				.where()
				.or(Expr.eq("status", "SUBMITTED"),
						Expr.or(Expr.eq("status", "APPROVED"),
								Expr.eq("status", "REJECTED")))
				.eq("empid", empId).ge("date", Application.getDate(date))
				.lt("endDate", Application.getNextDate(endDate, 1))
				.orderBy("date asc").findList());
	...
	public static List<Timesheet> checkHeadOfficeTimesheetExistsInEditForm(
			Long sid, Long empId, String date, String endDate, String sh,
			String sm, String eh, String em, Long tid) {
		return (find
				.where()
				.or(Expr.eq("status", "SUBMITTED"),
						Expr.or(Expr.eq("status", "APPROVED"),
	...
	public static List<Timesheet> checkTimesheetExistsInHoEmployeeEditForLeave(
			Long sid, Long empId, String date, String endDate, Long tid) {
		...
		return (find
				...
				.or(Expr.eq("status", "SUBMITTED"),
						Expr.or(Expr.eq("status", "APPROVED"),
								Expr.eq("status", "REJECTED")))
				.eq("firmid", sid).ne("id", tid).eq("empid", empId)
				.eq("date", Application.getDate(date))
				.orderBy("date asc").findList());
	}
	...
	public static List<Timesheet> checkHeadOfficeTimesheetExistsForLeave(
		...
		return (find
				...
				.or(Expr.eq("status", "SUBMITTED"),
						Expr.or(Expr.eq("status", "APPROVED"),
								Expr.eq("status", "REJECTED")))
				.eq("firmid", sid).eq("empid", empId)
				.ge("date", Application.getDate(date))
				.lt("endDate", Application.getNextDate(endDate, 1))
				.orderBy("date asc").findList());
	}
	...
	public static List<Timesheet> getHeadOfficeEmployeeTimeSheetForSubmitted(
			Long storeId) {
		List<Timesheet> timeSheetList = new ArrayList<Timesheet>();
	...
	public static List<Timesheet> getHeadOfficeEmployeeTimeSheetForSubmitted(
			Long storeId,Date stDate,Date enDate) {
		List<Timesheet> timeSheetList = new ArrayList<Timesheet>();
		timeSheetList = find.where().eq("firmid", storeId)
				.eq("status", "SUBMITTED").findList();
	...
	public static List<Timesheet> getHeadOfficeEmployeeTimeSheetForSubmittedByWeekDate(
			Long storeId,Date stDate,Date enDate) {
		List<Timesheet> timeSheetList = new ArrayList<Timesheet>();
		timeSheetList = find.where().eq("firmid", storeId)
				.eq("status", "SUBMITTED")
	...
	public static List<Timesheet> getHeadOfficeEmployeeTimeSheet(Long storeId) {
		List<Timesheet> timeSheetList = new ArrayList<Timesheet>();
		timeSheetList = find.where().eq("firmid", storeId)
				.eq("status", "APPROVED").findList();

		return timeSheetList;
	}

	public static String updateVerifiedHOList(Date sDate, Date eDate, Long empId) {
	...
	public static String updateRejectedListInHOTimesheet(Date sDate,
			Date eDate, Long empId) {
		Logger.info("updating records");
		String result = "";
		Timesheet t = new Timesheet();
		List<Timesheet> tmList = new ArrayList<Timesheet>();
	...
	public static void updateListForHOTInEditPage(Long tmId, Long storeId,
			Long empId, String startDate, String endDate, String stHours,
			String stMins, String endHours, String endMins, String duration,
			String activity, String leaveType) {
		Timesheet tm = Timesheet.find.byId(tmId);
		tm.id = tmId;
		tm.firmid = storeId;
		tm.empid = empId;
		tm.date = Application.getDate(startDate);
		tm.endDate = Application.getDate(endDate);
		tm.startTimeHour = stHours;
		tm.startTimeMins = stMins;
		tm.endTimeHour = endHours;
		tm.endTimeMins = endMins;
		tm.duration = duration;
		tm.activity = activity;
		tm.leaveType = leaveType;
		tm.status = "SUBMITTED";
		tm.update();
	}

	public static void updateHoEmployeeLeaveTypeInEditForm(Long tid, String leaveType) {
		Timesheet tm = Timesheet.find.byId(tid);
		tm.leaveType = leaveType;
		tm.startTimeHour = "0";
		tm.startTimeMins = "0";
		tm.endTimeHour = "0";
