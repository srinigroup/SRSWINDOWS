...
$("#ajaxform").submit(function(e)
 ...
 function accSalesHead(aid){
	...
	// calculate duration of timesheet of head office based on start and End time
	...
	// calculate duration of timesheet of edited head office based on start and End time
	
	function durationEditHeadOfficeTimeSheetFromStartEnd(){
	
		var startHours = document.getElementById('hoEditStartTimeHour');
		var startMins = document.getElementById('hoEditStartTimeMins');
		var endHours = document.getElementById('hoEditEndTimeHour');
		var endMins = document.getElementById('hoEditEndTimeMins');
		var startDate = document.getElementsByName('hoEditStartDate')[0];
		var endDate = document.getElementsByName('hoEditEndDate')[0];
		var date=document.getElementById("hoEditStartDate").value;
		var enddate=document.getElementById("hoEditEndDate").value;
			...
			if( (startDate.value != null) && (endDate.value != null) ){
				...
				if(sd.getTime() > ed.getTime()){ // if start Time greater than end time
					...
					if(sd.getDate() > ed.getDate() ){ // if start date is greater than end date
						...
						alert("End Date should be Greater Than Start Date");
						document.getElementsByName('hoEditEndDate')[0].focus();
						return false;
					}
					...
					if(sd.getDate() == ed.getDate()){ // if both dates are equal
						...
						if(startHours.value > endHours.value ){ // if both dates are equal check for hours
							...
							alert("End time should be Greater Than Start Time");
							document.getElementById('hoEditEndDate').focus();
							return false;
						...
						if(startHours.value == endHours.value ){ // if both hours are equal check for mins
							...
							if(startMins.value > endMins.value){
								...
								alert("End time should be Greater Than Start Time");
								document.getElementById('hoEditEndTimeMins').focus();
								return false;
							}
						}
					}
        			...
        			//console.log("duration is: "+ hours+":"+minutes);
 					...
 					//duration for HOE roles 
					if(document.getElementById('durationInHoEdit') != null){ // check for Sk timesheet ,sk duration have id  "durationForSK"
						
						document.getElementById('durationInHoEdit').value = hours+":"+minutes ;
					}else{													// check for SM timesheet , sm have id "duration"
	...
	//method for checking timesheets existing for HOT employess in edit page
	function checkHOTTimesheetExistsInEditPage(tid){ 
		
		var startHours = document.getElementById('hoEditStartTimeHour').value;
		var startMins = document.getElementById('hoEditStartTimeMins').value;
		var endHours = document.getElementById('hoEditEndTimeHour').value;
		var endMins = document.getElementById('hoEditEndTimeMins').value;
		var date = document.getElementsByName("hoEditStartDate")[0].value;
		var endDate = document.getElementsByName("hoEditEndDate")[0].value;
		var empId = document.getElementById('empid').value;
		var storeId =document.getElementsByName("firmid")[0].value;
		var leaveType = document.getElementById('leaveTypeInHOEdit').value;
		var hOTEditTextarea=document.getElementById('hOTEditTextarea').value;
		//console.log("leaveType : "+leaveType);
		//console.log("tid : "+tid);
		var flag = false ;
		if(date == "" || date.length == 0){
			// alert("Please select date");
			document.getElementById("hoEditDateErrorLabel").style.display="block";
			document.getElementById("startTimeHourEditErrorLabel").style.display="none";
			document.getElementById("startTimeMinsEditErrorLabel").style.display="none";
			document.getElementById("endTimeHourEditErrorLabel").style.display="none";
			document.getElementById("endTimeMinsEditErrorLabel").style.display="none";
			document.getElementById("hoActivityEditErrorLabel").style.display="none";
			document.getElementById("hoEditStartDate").focus();
		}
		else if(startHours == "" || startHours.length == 0){
			document.getElementById("startTimeHourEditErrorLabel").style.display="block";
			document.getElementById("hoEditDateErrorLabel").style.display="none";
			document.getElementById("startTimeMinsEditErrorLabel").style.display="none";
			document.getElementById("endTimeHourEditErrorLabel").style.display="none";
			document.getElementById("endTimeMinsEditErrorLabel").style.display="none";
			document.getElementById("hoActivityEditErrorLabel").style.display="none";
		...
		else if(startMins == "" || startMins.length == 0){
			document.getElementById("startTimeMinsEditErrorLabel").style.display="block";
			document.getElementById("hoEditDateErrorLabel").style.display="none";
			document.getElementById("startTimeHourEditErrorLabel").style.display="none";
			document.getElementById("endTimeHourEditErrorLabel").style.display="none";
			document.getElementById("endTimeMinsEditErrorLabel").style.display="none";
		...
		else if(endHours == "" || endHours.length == 0){
			document.getElementById("endTimeHourEditErrorLabel").style.display="block";
			document.getElementById("hoEditDateErrorLabel").style.display="none";
			document.getElementById("startTimeHourEditErrorLabel").style.display="none";
			document.getElementById("startTimeMinsEditErrorLabel").style.display="none";
			document.getElementById("endTimeMinsEditErrorLabel").style.display="none";
		...
		else if(endMins == "" || endMins.length == 0){
			document.getElementById("endTimeMinsEditErrorLabel").style.display="block";
			document.getElementById("hoEditDateErrorLabel").style.display="none";
			document.getElementById("startTimeHourEditErrorLabel").style.display="none";
			document.getElementById("startTimeMinsEditErrorLabel").style.display="none";
			document.getElementById("endTimeHourEditErrorLabel").style.display="none";
		...
		else if(hOTEditTextarea == "" || hOTEditTextarea.length == 0){
			document.getElementById("hoActivityEditErrorLabel").style.display="block";
			document.getElementById("hoEditDateErrorLabel").style.display="none";
			document.getElementById("startTimeHourEditErrorLabel").style.display="none";
			document.getElementById("startTimeMinsEditErrorLabel").style.display="none";
			document.getElementById("endTimeHourEditErrorLabel").style.display="none";
			document.getElementById("endTimeMinsEditErrorLabel").style.display="none";
			document.getElementById("hOTEditTextarea").focus();
		}
		//form validation end
		...
		if(leaveType == 'None'){
			...
			//make an Ajax call
			 myJsRoutes.controllers.Application.checkHeadOfficeTimesheetExistsInEditPage(storeId,empId,date,endDate,leaveType,startHours,startMins,endHours,endMins,tid).ajax({
	   			 success : function(data) {
	    		...
	    		//return false;
	    			...
	    			if(data > 0){ // Ajax call returns count of Existing Timesheet for the same Time range for this employee
	    				...
	    				//alert("Else");
	    				document.getElementById('hOTEditTimesheetForm').submit(); // submit the form here
	    				flag = true;
	    			}
	    		}
			});
			...
			//make an Ajax call
			 myJsRoutes.controllers.Application.checkHOTTimesheetExistsInEditPageForLeave(storeId,empId,date,endDate,tid).ajax({
	   			 success : function(data) {
	...
	//ENABLE OR DISABLE LEAVETYPE IN HOT EDIT PAGE
	$(function(){ 
	$('#applyLeaveCheckBoxInHOEdit').change(function () {
    	if($("#applyLeaveCheckBoxInHOEdit").prop('checked') == true){
    	
    		$('#leaveType_fieldInHoEdit').css('display','block'); // display leave type list box
			$('#leaveTypeInHOEdit').prop('required',true);
			$('#leaveTypeInHOEdit option:first').val("");
			
			$('#HOTimesheetEditStartTime_field').css('display','none'); // hide time fields,duration
			$('#HOTimesheetEditEndTime_field').css('display','none');
			$('#HOTimesheetEditDuration_field').css('display','none');
			$('#hOTEditTextarea').css('display','none');
			$('#hOTEditTextareaLabel').css('display','none');
			$('#durationInHoEdit').css('display','none');
			
			$('#hoEditEndTimeHour').prop('required',false); // disable required attribute if leave selected
			$('#hoEditEndTimeMins').prop('required',false);
			$('#hoEditStartTimeMins').prop('required',false);
			$('#hoEditStartTimeHour').prop('required',false);
			$('#hOTEditTextarea').prop('required',false);
			
		}else{
			
			$('#HOTimesheetEditDuration_field').css('display','none');
			$('#leaveType_fieldInHoEdit').css('display','none'); 
			$('#leaveTypeInHOEdit').prop('required',false);
			$('#leaveTypeInHOEdit option:first').val("None");
			
			
			//un selected the previous selected value
			$('#leaveTypeInHOEdit option:selected').removeAttr( "selected" );
			
			$('#HOTimesheetEditStartTime_field').css('display','block'); // diaplay time fields,duration
			$('#HOTimesheetEditEndTime_field').css('display','block');
			$('#HOTimesheetEditDuration_field').css('display','block');
			$('#hOTEditTextareaLabel').css('display','block');
			$('#durationInHoEdit').css('display','block');
			$('#hoEditEndTimeHour').prop('required',true); // enable required attribute if leave not selected
			$('#hoEditEndTimeMins').prop('required',true);
			$('#hoEditStartTimeMins').prop('required',true);
			$('#hoEditStartTimeHour').prop('required',true);
			$('#hOTEditTextarea').prop('required',true);
			$('#hOTEditTextareaLabel').prop('required',true);
		}
 	});
   ...
   function callTheJsonp(url)
	...
	//Funtion wriiten for to prevent the submit button from double clicks in TimeSheet show page
				...
				$(function()
					{
					  $('#timesheetForm').submit(function(){
					    $("#btnSubmitInTimesheet", this).val("Please Wait...")
					      .attr('disabled', 'disabled');
					      setTimeout('$("#btnSubmitInTimesheet").removeAttr("disabled")', 1500);
					         $("#btnSubmitInTimesheet", this).val("Submit")
					    return true;
					  });
					});	
			...
			$(function()
					{
					  $('#timesheetForm').submit(function(){
					    $("#btnSubmit", this).val("Please Wait...")
					      .attr('disabled', 'disabled');
					      setTimeout('$("#btnSubmit").removeAttr("disabled")', 1500);
					      $("#btnSubmit", this).val("submit")
					    return true;
					  });
					});	
	...
	function addTimeSheetHours(empid){
		...
		//console.log("startTimeHour min is : "+startTimeMins);
		//console.log("End time hour is : "+endTimeHour);
		//console.log("End time minis : "+endTimeMins);
	...
	//METHOD FOR SHOW MODAL IN HOT APPROVE FOR SINGLE EMPLOYEE
	var approveForm;
	function showAlertForApprove(approveButton){
		approveForm=approveButton.form;
		$('#approveModel').modal("show");
		
	}
	//METHOD FOR SUBMIT FORM  IN HOT APPROVE FOR SINGLE EMPLOYEE
	function subApproveForm(){
		approveForm.submit();
	}
	//METHOD FOR SHOW MODAL IN HOT APPROVE FOR ALL EMPLOYEE'S IN LIST PAGE
	var approveAllFormInListPage;
	function showAlertForApproveAllInListPage(approveAllButtonInLitPage){
		approveAllFormInListPage=approveAllButtonInLitPage.form;
		$('#approveAllModelInListPage').modal("show");
		
	}
	//METHOD FOR SUBMIT FORM  IN HOT APPROVE FOR ALL EMPLOYEE'S IN LIST PAGE
	function subApproveAllFormInListPage(){
		approveAllFormInListPage.submit();
	}
...
var headOfficeEmpTimesheetStartDate;
var headOfficeEmpTimesheetEndDate;
var headOfficeEmpTimesheetEmpId;
function updateRejectedHOTimesheets(weekStartDate,endDate,empId){
	headOfficeEmpTimesheetStartDate=weekStartDate;
	headOfficeEmpTimesheetEndDate=endDate;
	headOfficeEmpTimesheetEmpId=empId;
	console.log("weekStartDate :"+weekStartDate);
	console.log("endDate :"+endDate);
	document.getElementById("stDateForUpdateRejectedStatusForHOTimesheetsMethod").value=weekStartDate;
	document.getElementById("empIdForUpdateRejectedStatusForHOTimesheetsMethod").value=empId;
	$('#rejectModel').modal("show");

}
function subRejectForm(){
	document.getElementById("updateRejectedStatusForHOTimesheetFormId").submit();
	
} 
var weekStartDateInListPage;
var weekEndDateInListPage;
var empIdInListPage;
function showApproveModalInListPage(weekStartDate,endDate,empId){
	weekStartDateInListPage=weekStartDate;
	weekEndDateInListPage=endDate;
	empIdInListPage=empId;
	console.log("start Date : "+weekStartDate);
	console.log("endDate: "+endDate);
	console.log("empId : "+empId);
	document.getElementById("stDateForVerifyHOTimesheet").value=weekStartDate;
	document.getElementById("enDateForVerifyHOTimesheet").value=endDate;
	document.getElementById("empIdForVerifyHOTimesheet").value=empId;
	$('#approveModelInListPage').modal("show");
}
 
 function showMethodForGetHeadOfficeTimesheetsForVerify(empId,stDate){
 	console.log("empId is : "+empId);
  	console.log("stDate is : "+stDate);
 	document.getElementById("stDateForGetHeadOfficeTimesheetsForVerifyMethod").value=stDate;
 	document.getElementById("empIdForGetHeadOfficeTimesheetsForVerifyMethod").value=empId;
 	document.getElementById("subFormForGetHeadOfficeTimesheetsForVerifyMethod").submit();
 }